...
**java当中的泛型是伪泛型，进去的时候还是Object，出来的时候才去强制转换**
...
存储字符串并遍历 JDK8后 哈希值+链表+红黑树 

+ 加载因子，hashset的扩容时机
  + 长度16 ，因子0.75，如果数组里面超过16 * 0.75，那么数组扩容两杯 
  + 如果链表长度 >8 数组长度>64 时，对应index的数组变为红黑树
  + 链表插入，新元素直接挂在老元素下面
  + 集合当中时自定义对象，要重写 hashcode和equals
...
作用，把所有的节点都通过双向链表连接起来，保证存的顺序和取的顺序是一样的，也就是打印顺序和存取顺序一样
