...
**PATH=/bin:/sbin:/user/bin:/user/sbin:/system/Library/ export PATH mac当中设置环境变量**
...
**/usr/libexec/java_home -V 显示mac当中java安装在哪个目录**
...
**touch HelloWorld.java** mac创建文件夹
...
>
> ​	java + 文件名（运行编译之后的class文件） **注意，这里不能加后缀！**
...
​	是其他两个版本的基础。 **可以写桌面应用（有点慢，没有C快，主要是打基础）**
...
JVM（Java Virtual Machine），Java虚拟机，运行的地方

JRE（Java Runtime Environment），Java运行环境，包含了JVM和Java的核心类库（Java API） 

核心类库 比如System，out这种关键字

JDK（Java Development Kit）称为Java开发工具，包含了JRE和开发工具

开发工具:javac java这种工具库（javac java jdb jhat）

运行工具 开发工具的子集，只用到运行需要的工具（比如java）
