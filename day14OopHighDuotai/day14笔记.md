...
+ 局部代码块：用于节约内存，提前结束一个变量的生存周期
+ 构造代码块：多个构造方法中重复的代码写在构造代码块中，在构造方法执行之前会先执行这个代码块（写法，在构造方法上面整个大括号，写入对应代码即可）
+ 静态代码块，随着类的加载而加载，只执行一次。在类加载的时候，做一些数据初始化的时候使用
