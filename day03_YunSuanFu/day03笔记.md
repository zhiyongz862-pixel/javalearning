...
* 放在变量的前面，我们叫做先++。 比如：++a **先加再用**
* 放在变量的后面，我们叫做后++。 比如：a++ **先用再加**
...
存储数的时候，8位数，第一位符号位，后面是运算位，符号位为0代表正数，1代表负数

做运算时，负数要变成反码（也就是符号位不变，其他取反），正数不需要改变。

但是直接反码计算跨0的时候会产生误差（-4 + 7 会得到2而不是3，因为反码里面有正0和负0）
