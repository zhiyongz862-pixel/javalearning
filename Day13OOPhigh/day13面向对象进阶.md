...
+ this(),可以访问本类的构造函数
+ 如果没有构造方法，JVM会自动创建一个空参构造

+ 子类只能访问父类非私有成员
+ 继承相关
  + 构造方法子类不能继承 
  + 成员变量子类都能继承（但是不一定能调用，比如private定义的父类成员变量）
  + 成员方法是否可以继承？
    + 首先了解虚方法表的概念，java会从每个父类开始创建一个虚方法表，虚方法表里面的方法必须满足三点要求，然后子类调用方法时会去虚方法表里面去找。没找到才去逐个父类去找。只有虚方法才能被子类继承
      + 不能是private
      + 不能是static
      + 不能是final
+ 访问相关
  + 继承中成员变量的访问特点：就近原则，谁离我近就继承谁
  + 继承中成员方法的访问特点：就近原则，和成员变量一样
+ 方法重写相关
  + 成员方法的重写其实就是虚方法表的重写
  + 重写方法尽量和父类一样
  + 虚方法表里的方法才可以重写
+ 继承中构造方法相关
  + 父类中的构造方法不会被子类继承，但是子类所有的构造方法默认先访问父类的无参构造，再执行自己
  + 如果想调用父类的有参构造，必须自己在子类中写一个有参构造，然后super（k个参数），调用有参构造。（注意没有继承构造方法，但是可以通过super调用）
